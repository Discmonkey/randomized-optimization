...
INPUT_FILE = os.path.join("..", "datasets", "cache", "train_basketball_no_period.csv")
...
def initialize_instances():
    ...
    with open("datasets/cache/train_basketball.csv", "r") as abalone:
...
def get_max_index(something):
    max_item = 0
    max_index = -1
    for num, item in enumerate(something):
        if item > max_item:
            max_item = item
            max_index = num
...
def main():
    ...
    for _ in oa_names:
        classification_network = factory.createClassificationNetwork([9, 9, 9, 9, 10, 20, 30])
    ...
    print a, b, c
    oa.append(RandomizedHillClimbing(a))
    oa.append(SimulatedAnnealing(1E11, .95, b))
    oa.append(StandardGeneticAlgorithm(200, 100, 10, c))
    ...
    for i, name in enumerate(oa_names):
        ...
        for instance in instances:
            ...
            actual = get_max_index(networks[i].getOutputValues())
